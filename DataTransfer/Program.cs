...
namespace DataTransfer
{
    internal class Program
    {
        ...
        private static void TransferData(OleDbConnection accessConnection, ClientConnectModelContainer modelContainer)
        {
           ...
           //kolan connection
            using (OleDbCommand oleDbCommand = new OleDbCommand("select * from Kolan", accessConnection))
            {
                OleDbDataReader reader = oleDbCommand.ExecuteReader();
                while (reader.Read())
                {
                    string kolanSystemName = (reader["SysName"] as string);
                    if (kolanSystemName != null && (Regex.IsMatch(kolanSystemName.ToLower(), @"\bkolan\b")))
                    {
        ...
        static  string GetParDataStop(string parityId, OleDbConnection connection)
                {
             ...
             static int GetBaudRate(string baudRateId, OleDbConnection connection)
                {
                    using (OleDbCommand oleDbCommand =
            ...
            static string GetMachineTypeComment(string machineId, OleDbConnection connection)
                {
                    using (OleDbCommand oleDbCommand =
        ...
        static OleDbConnection ConnectToAccess()
                {
